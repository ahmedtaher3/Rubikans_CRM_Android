...
class HandBookViewHolder(itemView: View) : ParentViewHolder(itemView) {
    ...
    fun bind(item: HandBookItem , position:Int) {

        if (item.subs.isNullOrEmpty())
            itemView.findViewById<ImageView>(R.id.iv_arrow_expand).visibility = View.GONE

        itemView.findViewById<TextView>(R.id.handbook_title).text =
            "${position+1} - ${HtmlCompat.fromHtml(item.title!!, HtmlCompat.FROM_HTML_MODE_LEGACY)}"
    }
