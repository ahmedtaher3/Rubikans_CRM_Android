...
class WorkFromHomeFragment : BaseFragment<FragmentWorkFromHomeBinding>() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        binding.startWorkFromHome.setOnClickListener {
            ...
            if (!started)
                viewModel.startWorkFromHome(
                    "insert",
                    viewModel.dataManager.user.empId.toString(),
                    "",
                    viewModel.dataManager.user.nameAr.toString(),
                    viewModel.dataManager.user.managerId.toString(),
                    "",
                    "",
                    "",
                    ""
    ...
    private fun setObservers() {
        ...
        viewModel.responseLiveCheckWorkFromHome.observe(
            viewLifecycleOwner,
            androidx.lifecycle.Observer {
                ...
                if (it.isSuccessful) {
                    ...
                    binding.startWorkFromHome.setText(
                        "Started At \n " + it.workFromHome?.startedAt?.takeLast(
                            8
                        )
                    )
                    ...
                    if (it.workFromHome?.ended!!) {
                        ended = true
                        binding.startWorkFromHomeEnded.setText(
                            "Ended At \n " + it.workFromHome?.endedAt?.takeLast(
                                8
                            )
                        )
